...
You have access to a personal knowledge vault through three MCP tools: `kg_recall`,
`kg_file`, and `kg_extract`. Use them proactively — do not wait to be asked.

### At session start

When the user's first message reveals a topic you may have covered before, immediately
call `kg_recall` with 2–4 relevant terms before responding. Do this silently and
...
- User mentions a project name, technology, or tool → `kg_recall("project-name technology")`
- User describes a problem type you may have solved before → `kg_recall("error type system")`
- User continues work from a previous session → `kg_recall("topic keywords")`

### Mid-session

When the conversation shifts to a new topic or the user asks about something you may
have prior notes on, call `kg_recall` with relevant terms. Surface the recalled context
...
"add to my notes", or similar — immediately call `kg_file` with the information.
...
1. This prompt works best when your vault has notes from several previous sessions.
   Run `/kg-extract` in a Claude Code session (or use the `kg_extract` MCP tool) to
   build up initial vault content.

2. If `kg_recall` returns no results for a topic, that's expected for new topics.
   The vault grows over time as sessions are captured.

3. For project-specific routing: set a `cwd` parameter in `kg_file` calls to route
   notes to your project's vault folder (requires `.claude/knowledge.local.json` in

...
**P3 — The vault defines the schema.** The vault's `CLAUDE.md` is the authoritative source of type vocabulary, filing conventions, tag structures, and folder organization. The system reads it before filing anything. When there is no `CLAUDE.md`, the system tells the user to run `/kg-setup` first, and falls back to a minimal four-type default for any immediate filing needs. That bootstrap default does not apply once `CLAUDE.md` exists.

**P4 — Commands invoke themselves.** Skills should be written so that Claude triggers them without the user having to know the command name. `/kg-recall` fires when the user needs context to fulfill a request — when they ask about something they may have encountered before, or when working on a topic with known vault history. It does not fire automatically at session start; there must be a concrete information need to retrieve against. `/kg-file` triggers on "save this" or "remember that." `/kg-extract` runs at session boundaries. Utility commands like `/kg-setup` and `/kg-enrich` fire when the user describes what they want in plain language.
...
- Five slash command skills for Claude Code: `/kg-recall`, `/kg-file`, `/kg-extract`, `/kg-setup`, `/kg-enrich`
...
2. Suggest running `/kg-setup` to generate one
...
**How CLAUDE.md is used:** The full text of CLAUDE.md is passed as context to the LLM alongside the content to file or classify. The LLM extracts type vocabulary, folder rules, and naming conventions from the prose. This means the CLAUDE.md generated by `/kg-setup` must be written clearly enough for the model to reliably identify the type vocabulary and filing rules without structural parsing. `/kg-setup` is responsible for generating CLAUDE.md in a format that works for this use case.
...
**Invocation:**
```
/kg-recall redis cache eviction policy
/kg-recall auth token expiry
/kg-recall                    # no query: infer from current conversation context
```

**Behavior:**

1. Read vault path from `~/.claude/knowledge.json`.
2. Check for `.claude/knowledge.local.json` up the directory tree to find project vault folder.
...
4. Build search scope: project folder first (ranked higher), then inbox and staging folders.
...
**No-results behavior:** Say so clearly. Suggest `/kg-extract` to capture the current session, or `/kg-file` to save something specific.
...
**Note:** `/kg-recall` should not fire at session start without a concrete information need. Recall requires context to retrieve against — "what does the user need right now?" drives the query, not the fact that a session has started.
...
**Invocation:**
```
/kg-file The retry logic silently drops jobs within the 30s window — design, not a bug
/kg-file --type decision --folder Work/Areas/hermes The auth middleware runs before rate limiting
/kg-file                    # no content: extract from recent conversation context
/kg-file --dry-run [content]  # preview beats and routing without writing
```
...
/kg-extract --dry-run                               # preview current session without writing
/kg-extract ~/.claude/projects/.../abc123.jsonl     # specific transcript
/kg-extract --dry-run ~/.claude/projects/.../abc123.jsonl
/kg-extract ~/Downloads/chatlog.txt                 # plain text file
```
...
**Using dry-run to preview the PreCompact hook:** Running `/kg-extract --dry-run` on the current session is the primary way to inspect what the PreCompact hook *would* capture before running `/compact`. Do this when tuning extraction quality.
...
3. **Check deduplication:** if this session ID is already in `~/.claude/logs/kg-extract.log`, skip and report. Don't re-extract already-processed sessions. If the log file is unreadable or corrupt, warn and proceed (do not block extraction).
...
5. File each beat per the same routing logic as `/kg-file` (autofile or inbox drop, per CLAUDE.md conventions).
6. Write log entry to `~/.claude/logs/kg-extract.log`. Format: one entry per line, tab-separated: `<ISO-timestamp>\t<session-id>\t<beat-count>`. Example: `2026-03-01T14:32:00\tabc12345\t5`
...
**Invocation:**
```
/kg-setup                                              # vault_path from ~/.claude/knowledge.json
/kg-setup ~/Documents/my-vault                         # explicit path
/kg-setup --types "decision, insight, problem, note"   # specify type vocabulary directly
/kg-setup --dry-run                                    # show what CLAUDE.md would contain without writing it
```
...
1. Determine the vault path: from argument, or from `~/.claude/knowledge.json`. If neither, ask.
...
4. Run the vault analyzer script (`skills/kg-setup/scripts/analyze_vault.py`) to collect structural data: folder tree, frontmatter field usage, type distribution, tag patterns, naming conventions, link density, hub notes, orphan count. This script already exists (migrated from `skills/kg-claude-md/scripts/analyze_vault.py`). If it fails for any reason, continue with a degraded analysis using only in-context file reads — do not abort.
...
6. **Include the beat-to-vault-type mapping** if the vault uses its own type vocabulary (whole-life PKM archetype). Claude needs to know: when `/kg-extract` produces an `insight` beat, which vault type does it file under? This mapping must be explicit and unambiguous.
...
5. **One recommended next action**: e.g., "Run `/kg-enrich --dry-run` to see how many notes are missing metadata" or "Consider consolidating `project` and `task` types — the proposed migration path is in the Known Issues section."
...
**Purpose:** Scan the vault for notes missing required metadata and enrich them so they're findable by `/kg-recall`. Bridges the gap between rough human-authored notes and well-structured beats.

**Invocation:**
```
/kg-enrich                          # scan entire vault
/kg-enrich --folder Work/Inbox      # specific folder
/kg-enrich --dry-run                # report what would change, don't modify
/kg-enrich --since 2026-02-01       # only notes modified since date
/kg-enrich --overwrite              # replace existing fields, not just fill gaps
```
...
| Form | Example |
|---|---|
| Explicit flag | `/kg-extract --dry-run` |
...
**`/kg-extract --dry-run`**
Full output format as above. Also the primary way to preview what the PreCompact hook *would* do on the current session — run `/kg-extract --dry-run` before `/compact` to inspect the extraction quality.
...
5. Log outcome to `~/.claude/logs/kg-extract.log`.
...
**Condition for inclusion in V1:** Requires confirming that Claude Code fires a `stop` event hook with usable context (transcript path, session ID, cwd). If this hook is available and reliable, implement it. If not, document the gap and the manual workaround (`/kg-extract` before closing).
...
Both formats feed through the same beat extraction pipeline used by `/kg-extract`.
...
- Route beats per the same logic as `/kg-extract` (autofile or inbox drop)
...
### Per-project: `.claude/knowledge.local.json`
...
The extractor walks up the directory tree from the session's working directory to find the nearest `knowledge.local.json`.
...
**Config:** `"autofile": true` in `~/.claude/knowledge.json`
...
- The journal file itself is excluded from `/kg-enrich` enrichment (it is not a beat)
- Journal entries are append-only; the extractor never modifies existing entries
- The journal is a passive log, not a retrievable beat — it is not indexed by `/kg-recall`
...
| `/kg-recall` output injected into session | Vault notes (user-authored + auto-extracted) | Active session LLM | Full tool access | **High** — active session can execute tools |
| Vault `CLAUDE.md` passed to extraction LLM | User-authored + `/kg-setup` generated | Extraction/autofile model | None | Low — but CLAUDE.md is more trusted than vault notes; a compromised CLAUDE.md could reshape all filing decisions |
| Import pipeline | External exports (ChatGPT, Claude) | Extraction model | None | Low |
| `/kg-file` content | User input + recent conversation | Extraction model | None | Low |
...
When `/kg-recall` or `kg_recall()` injects vault content into an active session, the output must be wrapped in a clear structural boundary:
...
`~/.claude/knowledge.json` contains the vault path and other sensitive config. No additional permission requirements are imposed in V1 (the file already lives in `~/.claude/` which is user-private). The system must validate that `vault_path` is not the user's home directory or the filesystem root — these indicate misconfiguration that would result in beats written to unexpected locations. If `vault_path` equals `$HOME` or `/`, exit with a clear error message.
...
- `--folder ../../../etc` in `/kg-file` → rejected
...
| `references/ontology.md` in kg-file skill | Replaced by vault CLAUDE.md as the type authority |
...
|---|---|---|
| `/kg-recall [query]` | "What do I know about X?" | Search vault, inject relevant context |
| `/kg-file [content]` | "Save this." / "Remember this." | Extract beats from content and file to vault |
| `/kg-extract [path?]` | "Capture this session." | Extract all beats from a session transcript |
| `/kg-setup [vault?]` | "Set up my vault." | Analyze vault, generate or update CLAUDE.md |
| `/kg-enrich [options]` | "Clean up the vault." | Enrich notes missing metadata |

### When Claude should invoke each skill automatically

| Situation | Skill invoked |
|---|---|
| User says "save this", "remember this", "file this" | `/kg-file` |
| User asks about prior decisions, past approaches, or prior work on a topic | `/kg-recall` |
| A request would benefit from prior context and vault history exists for this project | `/kg-recall` |
| User says "set up my vault", "analyze my vault", "update my CLAUDE.md" | `/kg-setup` |
| User says "clean up the vault", "enrich my notes" | `/kg-enrich` |
| User says "save this session", "capture this before I close" | `/kg-extract` |
...
| `/kg-file` ontology | Reads separate `references/ontology.md` with 13 types | Reads vault's CLAUDE.md; no separate ontology file |
...
| `/kg-recall` return | Loads full note content for top matches | Summary cards for all; full body for 1–2 most relevant only |
| `/kg-claude-md` | Named after mechanism; no clarifying questions | Renamed `/kg-setup`; asks before generating when conventions are ambiguous |
...
| Dry-run | `/kg-enrich` only | All writing operations: `/kg-file`, `/kg-extract`, `/kg-setup`, `/kg-enrich`, import scripts |
...
1. `/kg-setup` analyzes a vault and generates a CLAUDE.md the user considers accurate, asking the right clarifying questions when conventions are ambiguous
2. `/kg-file` saves a piece of knowledge in minimal user time, landing in the right place per CLAUDE.md conventions, with no required manual type or folder selection
3. `/kg-recall` returns relevant notes without loading full note bodies unless necessary; results feel like remembering, not searching
4. `/kg-extract` extracts a proportionate number of beats from a session (selective, not exhaustive) and files them correctly
5. `/kg-enrich` processes a folder of manually-added notes and adds correct type, summary, and tags per CLAUDE.md conventions
